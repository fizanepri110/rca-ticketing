...
export default function EditEventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const { data: event, error: evErr } = await supabaseBrowser
          .from('events')
          .select(`*, ticket_types(*)`)
          .eq('id', eventId)
          .single()
        ...
        setHeure(event.date.split('T')[1].substring(0, 5))
        setDescription(event.description)
        setStatut(event.statut)
        setImageUrl(event.image_url || '')
        setImagePreview(event.image_url || null)

        const initialTickets = (event.ticket_types || []).map((t: any) => ({
          id: uid(),
          nom: t.nom,
          prix: t.prix.toString(),
          quantite_max: t.quantite_max.toString(),
        }))
        setTickets(initialTickets.length > 0 ? initialTickets : [{ id: uid(), nom: 'Standard', prix: '5000', quantite_max: '200' }])

      } catch (e: any) {
        setSubmitError(e.message ?? 'Erreur de chargement')
      } finally {
        setIsReady(true)
      }
    }
  ...
  function validateBillets(): string | null {
    ...
    for (const t of tickets) {
      ...
      if (!t.quantite_max || parseInt(t.quantite_max) <= 0) return `Quantité invalide pour "${t.nom}".`
    }
    return null
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const capacite = tickets.reduce((sum, t) => sum + parseInt(t.quantite_max || '0'), 0)
      ...
      const { error } = await supabaseBrowser.rpc('update_event_with_tickets', payload)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-blue-700 text-white px-6 py-4 flex items-center gap-4">
        ...
        <div>
          ...
          <h1 className="font-bold text-lg leading-tight">Éditer l'événement</h1>
        </div>
      </header>
                ...
                {imagePreview ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-48 gap-3 text-gray-400">
                    ...
                    <div className="text-center">
                      <p className="text-sm font-medium text-gray-600">Glissez une image ici ou cliquez</p>
            ...
            <div className="bg-white rounded-3xl shadow-sm p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex : Soirée Gala de la Paix"
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-3xl shadow-sm p-6">
              ...
              <p className="text-xs text-gray-500 mb-5">
                La mise à jour des billets va recréer les tickets. 
                S'assurer que cela n'affecte pas les ventes existantes.
              </p>
              ...
              <div className="flex gap-2 mb-5 flex-wrap">
                ...
                {TICKET_PRESETS.map((p) => (
                  <button
                    ...
                    type="button"
                    onClick={() => addTicket(p)}
                    className="text-xs font-semibold px-3 py-1.5 rounded-full border border-gray-200 bg-gray-50 hover:bg-blue-50 transition"
                  >
                    + {p.nom}
                  </button>
                ))}
              ...
              <div className="space-y-3">
                {tickets.map((ticket, idx) => (
                  <div key={ticket.id} className="border border-gray-100 rounded-2xl p-4 bg-gray-50 relative">
            ...
            <div className="bg-white rounded-3xl shadow-sm overflow-hidden">
              ...
              <div className="p-6 space-y-3">
                ...
                <p className="text-sm text-gray-500 flex items-center gap-1.5">
                  ...
                  {date && heure ? new Date(`${date}T${heure}`).toLocaleDateString('fr-FR', {
        ...
        <div className="flex gap-3 mt-6">
          ...
          {step !== 'recap' ? (
            <button
              type="button"
              onClick={handleNext}
              className="flex-1 py-3.5 rounded-2xl bg-blue-600 text-white font-semibold text-sm hover:bg-blue-700 transition"
            >
              Suivant →
            </button>
          ) : (
            <button
              ...
              className="flex-1 py-3.5 rounded-2xl bg-blue-600 text-white font-bold text-sm hover:bg-blue-700 transition flex items-center justify-center gap-2 disabled:opacity-60"
            >
              {submitting || imageUploading ? (
                <>
                  <Loader2 size={18} className="animate-spin" />
                  {imageUploading ? 'Upload image…' : 'Mise à jour…'}
                </>
              ) : (
                <>
