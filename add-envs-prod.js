...
// IMPORTANT: Replace placeholder values with actual credentials before running this script
const envs = {
...
for (const [key, val] of Object.entries(envs)) {
  try {
    ...
    execSync(`node -e "process.stdout.write('${val}')" | vercel env add ${key} production --scope nel667s-projects`, { stdio: 'inherit' });
  } catch (err) {
    console.error(`Failed to add ${key} for production (might already exist)`);
    // if exists, let's remove it and add again
    try {
        console.log(`Removing existing ${key}...`);
        execSync(`vercel env rm ${key} production -y --scope nel667s-projects`, { stdio: 'ignore' });
        execSync(`node -e "process.stdout.write('${val}')" | vercel env add ${key} production --scope nel667s-projects`, { stdio: 'inherit' });
    } catch(e) {
        console.error(`Double fail ${key}`);
    }
  }
}
